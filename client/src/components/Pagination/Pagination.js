...
const Pagination = () => {
    ...
    function changePage(path, page) {
        ...
        // }
        if (pathname.includes('order') && path === 'Africa' || path === 'Americas' || path === 'Asia' || path === 'Europe' || path === 'Oceania') {
            dispatch(filterRegion(path, page));
        }
        if (pathname.includes('order') && path === 'AtoZ' || path === 'ZtoA' || path === 'PopulationUp' || path === 'PopulationDown') {
        ...
        <>
            <div style={{ display: "flex" }}>
                {totalPages && totalPages.map(page =>
                    <button
                        key={page}
                        onClick={() => changePage(path, page - 1)}
                    >
                        {page}
                    </button>
                )}
