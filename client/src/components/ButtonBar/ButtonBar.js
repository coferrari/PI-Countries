...
const ButtonBar = () => {
  ...
  const handleChangeFilter = (e) => {
    ...
    // history.push(`/order/${e.target.value.replace(/ /g, "")}`);
    // e.target.value === "All"
    //   ? dispatch(getCountries("countries", 1))
    //   : dispatch(filterRegion(e.target.value, 0));
  };
  ...
  const handleChangeOrder = (e) => {
    ...
    // dispatch(orderCountries(e.target.value.replace(/ /g, ""), 0));
  };

  useEffect(() => {
    if (state === "All") dispatch(getCountries("countries", 1));
    if (
      state === "Africa" ||
      state === "Americas" ||
      state === "Asia" ||
...
//   const handleChange = (event, page = 1) => {
